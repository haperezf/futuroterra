...
def evaluar_modelos_regresion(modelos, X_test, y_test):
    ...
    return pd.DataFrame(resultados).sort_values(by="R2 Score", ascending=False)
...
def evaluar_estado(state):
    ...
    # Regresión
    input_reg_energy = np.array([
        sag_pressure, stockpile_level, solids_feeding, sump_level, sag_water,
        pebble, gran_gt_100, gran_lt_30, sag_speed, hardness, edad_liner,
        porcentaje_fino, p80
    ]).reshape(1, -1)
    ...
    input_clas = np.array([
        sag_pressure, stockpile_level, solids_feeding, sump_level, sag_water,
        pebble, gran_gt_100, gran_lt_30, sag_speed, hardness, edad_liner,
        porcentaje_fino, p80, consumo_energia_adjusted
    ]).reshape(1, -1)
...
def entrenar_modelos():
    ...
    features_reg_energy = [
        'Presion_SAG','Nivel_Stockpile','Solidos_en_alimentacion_mineral',
    ...
    mejor_modelo_reg_energy = resultados_reg_energy.iloc[0]['Modelo']
    mejor_modelo_reg_energy_obj = modelos_regresion_energy[mejor_modelo_reg_energy]

    # Usamos la red neuronal como mejor clasificador
...
app = Flask(__name__)
CORS(app)  # Habilita CORS para llamadas desde otro origen (frontend)

@app.route("/")
def index():
    return "Backend de SAG Operaciones - OK"
